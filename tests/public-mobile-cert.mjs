...
async function waitDeploy(){for(let i=0;i<36;i++){try{const r=await fetch(`${BASE}/?mobilecert=${Date.now()}`,{cache:'no-store'});const t=await r.text();if(r.ok&&t.includes('responsive.css?v=2026.08.26.004'))return;}catch{}await sleep(10000);}throw new Error('Mobile-polished release did not become live in time.');}
let browser;
try{
  await waitDeploy();
...
const result={certification:'ALLSHIELD public mobile homepage certification',base_url:BASE,completed_at:new Date().toISOString(),status:failures.length?'FAIL':'PASS',passed:checks.filter(x=>x.ok).length,total:checks.length,checks,failures};
