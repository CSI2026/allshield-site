...
const coreSupport=await readFile("support-agent-core.js","utf8");
for(const marker of ["support_target_user_id","supportSetOnboarding","supportCompleteModule","startLivePracticeExam","renderProduction"]) if(!coreSupport.includes(marker)) throw new Error(`Support-aware core agent marker missing: ${marker}`);
const crmSupport=await readFile("support-crm-overrides.js","utf8");
if(!crmSupport.includes("loadAgentCRM")||!crmSupport.includes("crm_log_activity")) throw new Error("Support-aware CRM is incomplete.");
const compSupport=await readFile("support-comp-overrides.js","utf8");
if(!compSupport.includes("loadMyCompensation")||!compSupport.includes("agent must personally accept")) throw new Error("Support-aware compensation/legal view is incomplete.");
...
const exec=await readFile("live-executive-dashboards.js","utf8");
if(!exec.includes("loadOwnerLiveDashboard")||!exec.includes("loadAdminLiveDashboard")) throw new Error("Live Owner/Admin dashboards are incomplete.");
const build = await readFile("build-info.js","utf8");
if(!build.includes("B2026.08.19.003") || !build.includes("allshieldBuildBadge")) throw new Error("Visible build identification is incomplete.");
