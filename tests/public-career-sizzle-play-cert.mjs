...
try{
  ...
  await page.waitForSelector('.shell',{timeout:30000});await page.waitForFunction(()=>typeof window.openCareersPage==='function',{timeout:30000});await page.evaluate(()=>window.openCareersPage());
  ...
  const meta=await page.locator('#careersPage .career-sizzle-frame video').evaluate(async v=>{
    ...
    if(v.readyState<1) await Promise.race([new Promise((resolve,reject)=>{v.addEventListener('loadedmetadata',resolve,{once:true});v.addEventListener('error',()=>reject(new Error(v.error?.message||'video error')),{once:true});}),new Promise((_,reject)=>setTimeout(()=>reject(new Error('metadata timeout')),30000))]);
  ...
  const playback=await page.locator('#careersPage .career-sizzle-frame video').evaluate(async v=>{v.muted=true;try{await v.play();}catch{}await new Promise(r=>setTimeout(r,1800));const out={currentTime:v.currentTime,paused:v.paused,ended:v.ended,error:v.error?{code:v.error.code,message:v.error.message}:null};v.pause();return out;});
