...
function injectStyles(){
  ...
  s.textContent=`
    ...
    #careersPage .career-sizzle-placeholder.career-sizzle-top{
      ...
      background:
        radial-gradient(circle at 78% 12%,rgba(64,163,236,.16),transparent 28%),
        linear-gradient(180deg,#040a12,#071321)!important;
      border-top:0!important;
    }
    #careersPage .career-sizzle-top .career-sizzle-card{
      padding:26px!important;
      border-radius:24px!important;
      background:linear-gradient(145deg,rgba(12,39,68,.95),rgba(5,15,27,.98))!important;
      box-shadow:0 30px 85px rgba(0,0,0,.34)!important;
    }
    #careersPage .career-sizzle-top .career-sizzle-card h2{
      font-size:clamp(34px,4.4vw,58px)!important;
      line-height:1.02!important;
      margin:9px 0 14px!important;
    }
    #careersPage .career-sizzle-top .career-sizzle-card p{
      font-size:15px!important;
      line-height:1.7!important;
      color:#a9bdd0!important;
      max-width:650px!important;
    }
    #careersPage .career-sizzle-top .career-sizzle-frame{
      min-height:300px;
    ...
    @media(max-width:820px){
      ...
      #careersPage .career-sizzle-top .career-sizzle-card{padding:16px!important;gap:14px!important}
...
function moveSizzleToTop(){
  ...
  if(left){
    const eyebrow=left.querySelector('.career-eyebrow');
    ...
    if(p) p.textContent='Before you read the rest of the Careers page, watch the opportunity first. This section is reserved for the finished studio-produced recruiting video and is linked directly to the ALLSHIELD Video Studio project.';
  }
}
...
function removeRobotNarrationLabels(){
  ...
  if(btn){
    btn.querySelectorAll('span').forEach(el=>{
      ...
      if(/Narrated/i.test(t)) el.textContent=t.replace(/\s*•?\s*Narrated\s*•?/i,' • Cinematic visual preview • ');
    });
  }
...
function install(){
  ...
  setInterval(()=>{ if(document.visibilityState==='visible') clean(); },2500);
}
