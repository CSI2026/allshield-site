(() => {
  ...
  window.loadProductionHealth=async function(){
    ...
    const approvedBrand=['brand-9aa0ec99b3b0.webp','brand-6553d9469f9e.webp'];
    const bad=[...document.images].filter(i=>{const src=String(i.getAttribute('src')||'');return /brand-[a-f0-9]+\.webp/i.test(src)&&!approvedBrand.some(x=>src.includes(x))});
    checks.push(['Approved logo',bad.length===0,bad.length?`${bad.length} visible non-homepage brand asset(s)`:'Homepage-approved Allshield assets enforced']);
