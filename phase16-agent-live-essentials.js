...
async function dashboard(){
  ...
  `<div class="real-data-banner">LIVE SUPABASE DATA</div>
   <div class="stat-grid" style="margin-top:18px">
    <div class="stat"><div class="label">ONBOARDING</div><div class="value">${onboardingPct}%</div></div>
    <div class="stat"><div class="label">LICENSE READINESS</div><div class="value">${readiness}%</div></div>
    <div class="stat"><div class="label">LATEST EXAM</div><div class="value">${latest===null?'—':latest+'%'}</div></div>
    <div class="stat"><div class="label">QUALIFIED RECORDS</div><div class="value">${qualified}</div></div>
   </div>
...
function install(){
  ...
  const safe=(fn)=>()=>Promise.resolve(fn()).catch(err=>{
    const main=h();
    if(main) main.innerHTML=`<div class="bo-card"><h3>Unable to load this section</h3><p>${esc(err.message||err)}</p></div>`;
  });
