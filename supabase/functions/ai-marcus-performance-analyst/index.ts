...
async function rows(table:string,select="*",limit=5000):Promise<any[]>{const r:any=await db.from(table).select(select).limit(limit);if(r.error)throw r.error;return (r.data||[]) as any[]}
