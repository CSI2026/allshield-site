(() => {
  ...
  window.loadProductionHealth=async function(){
    ...
    try{const {data,error}=await sb.from('curriculum_validation_findings').select('id',{count:'exact',head:false}).is('resolved_at',null).eq('severity','critical');checks.push(['Critical curriculum findings',!error&&(data||[]).length===0,error?error.message:`${(data||[]).length} open critical finding(s)`])}catch(e){checks.push(['Critical curriculum findings',false,e.message])}
    ...
    checks.push(['Owner Testing & Scores',typeof window.loadOwnerTestingOverview==='function',typeof window.loadOwnerTestingOverview==='function'?'Loaded':'Missing']);
